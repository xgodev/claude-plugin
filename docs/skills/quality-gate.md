...
- Explicit triggers: "run quality gate", "run QG", "run gate", "run the gate", "check quality", "check quality before PR", "validate before PR", "is it ready for PR", "qa before push".
- Auto-trigger: when the user says "I'll open a PR" / "open PR now" / "let's push", the skill offers to run the gate first (and asks if it's already been run).
...
6. **Render with analysis.** For every regressed/violated metric, read the matching `pr-<metric>.log` / `abs-<metric>.log`, cite `file:line`, suggest a specific fix.
7. **Behavior by verdict:** `passed` -> green + "OK to open the PR" (no `gh pr create`); `regressed`/`failed` -> table with analysis, offers to help fix, no `QG_BYPASS_REASON` suggestion; `bypassed` -> warning with declared reason + audit-log reminder.
...
- Supported languages: source of truth is the "Supported languages" table in the gate repo's `README.md`. Today: Rust, Go, Python, Node.js, Java, Swift, Kotlin, and **Web (HTML/CSS static)**. The `web` gate measures only `fmt`+`lint` and only fires when there is no `package.json` (React/Vue/etc. with `package.json` = a nodejs project).
