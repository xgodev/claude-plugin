...
Consumer of [`quality-gate`](https://github.com/xgodev/quality-gate). Calls the gate's **dispatcher** (`~/.quality-gate/qg`), which detects the project's language(s) 100% in shell (zero AI), runs the matching `<lang>/qg.sh` gate(s), parses the JSON verdict, and renders an analyzed result in PT-BR before the developer opens a PR. The gate lives in another repo and is cloned into `~/.quality-gate/` on first use.
...
1. **Cache the gate.** Skill ensures `~/.quality-gate/` exists (clones if missing) and runs `git fetch + pull --ff-only` on **every run** (the repo is tiny; this guarantees the gate is always the latest `main`, no staleness window). Override with `QG_PATH` for devs editing the gate locally. `--ff-only` failure (cache diverged) -> stop, instruct manual resolution.
...
- Gate repo (separate): `github.com/xgodev/quality-gate`. Cloned into `~/.quality-gate/` at runtime.
- Contract reference (after cloning the gate): `~/.quality-gate/docs/contract.md` (dispatcher, tamper-resistance, `.qg.yaml projects:`), `docs/output-format.md` (monorepo envelope), `docs/consume.md`.
...
- Cache updates on **every run** (`git fetch + pull --ff-only`); no staleness window. `QG_PATH` overrides for local gate development.
