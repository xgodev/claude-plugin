...
#
# nodejs/qg.sh -- Quality Gate for Node.js
#
...
show_help() {
  cat <<'EOF'
Usage: nodejs/qg.sh [--base <git-ref>] [options]
     nodejs/qg.sh --detect
...
check_prereqs() {
  local missing=()
  command -v node >/dev/null 2>&1 || missing+=("node (Node.js 18+) -- install: 'curl -fsSL https://deb.nodesource.com/setup_lts.x | sudo -E bash - && sudo apt install -y nodejs' (Linux) / 'brew install node' (macOS) (without Node no metric runs)")
  command -v npm >/dev/null 2>&1 || missing+=("npm (comes with Node.js) -- install: 'sudo apt install -y nodejs npm' (Linux) / 'brew install node' (macOS) (without npm there is no dependency resolution)")
  command -v npx >/dev/null 2>&1 || missing+=("npx (comes with Node.js) -- install: 'sudo apt install -y nodejs npm' (Linux) / 'brew install node' (macOS) (without npx prettier/eslint/tsc/c8 do not run)")
  ...
  # prettier, eslint and c8 are installed on demand via 'npx --yes' (cached in ~/.npm/_npx).
  # We do not validate a specific version -- it depends on what the target project uses.
...
if [ "$QG_ABSOLUTE_MODE" = "1" ]; then
  ...
  if ! qg_resolve_deps "." "$QG_LOG_DIR_ARG/abs-deps.log"; then
    echo "::error::failed to resolve nodejs dependencies -- see $QG_LOG_DIR_ARG/abs-deps.log" >&2
    exit 2
  fi
...
NODE_PATH_RE='\.(js|jsx|ts|tsx|mjs|cjs)$|^package(-lock)?\.json$|^tsconfig\.json$|^pnpm-lock\.yaml$|^yarn\.lock$'

if [ -n "$QG_YAML_EXTRA_FAST_PATH" ]; then
  NODE_PATH_RE_EXTRA="${NODE_PATH_RE}${QG_YAML_EXTRA_FAST_PATH}"
else
  NODE_PATH_RE_EXTRA="$NODE_PATH_RE"
fi
...
if [ "$QG_FORCE_FULL_ARG" != "1" ]; then
  ...
  if [ -n "$changed_files" ] && ! echo "$changed_files" | grep -qE "$NODE_PATH_RE_EXTRA"; then
    branch=$(git rev-parse --abbrev-ref HEAD 2>/dev/null || echo "<detached>")
    ts=$(date -u +"%Y-%m-%dT%H:%M:%SZ")
...
OK fast-path passed (nenhum Node.js para medir)
EOF
    fi
...
if [ -z "$QG_BASELINE_DIR_ARG" ]; then
  ...
  cache_root="${QG_BASELINE_CACHE_DIR:-/tmp/qg-baseline-nodejs}"
...
# Language absent in baseline (sentinela: package.json)
if [ ! -f "$QG_BASELINE_DIR_ARG/package.json" ]; then
  branch=$(git rev-parse --abbrev-ref HEAD 2>/dev/null || echo "<detached>")
  ts=$(date -u +"%Y-%m-%dT%H:%M:%SZ")
  if [ "$QG_FORMAT_ARG" = "json" ]; then
    cat <<EOF
...
EOF
    ...
    echo "::warning::language absent in baseline (package.json not found in $QG_BASELINE_DIR_ARG) -- gate skipped" >&2
  fi
  exit 0
fi
...
if ! qg_resolve_deps "$QG_BASELINE_DIR_ARG" "$QG_LOG_DIR_ARG/base-deps.log"; then
  echo "::error::failed to resolve nodejs dependencies (baseline) -- see $QG_LOG_DIR_ARG/base-deps.log" >&2
  exit 2
fi
if ! qg_resolve_deps "." "$QG_LOG_DIR_ARG/pr-deps.log"; then
  echo "::error::failed to resolve nodejs dependencies (PR) -- see $QG_LOG_DIR_ARG/pr-deps.log" >&2
  exit 2
fi
