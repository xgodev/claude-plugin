...
#
# kotlin/qg.sh -- Quality Gate for Kotlin (Gradle)
#
...
show_help() {
  cat <<'EOF'
Usage: kotlin/qg.sh [--base <git-ref>] [options]
     kotlin/qg.sh --detect
...
check_prereqs() {
  ...
  command -v gradle >/dev/null 2>&1 || missing+=("gradle -- install: 'sdk install gradle' via SDKMAN (Linux) / 'brew install gradle' (macOS) (without Gradle there is no way to measure the build; prefer committing ./gradlew)")
  command -v ktlint >/dev/null 2>&1 || missing+=("ktlint -- install: 'curl -sSLO https://github.com/pinterest/ktlint/releases/latest/download/ktlint && chmod +x ktlint && sudo mv ktlint /usr/local/bin/' (Linux) / 'brew install ktlint' (macOS) (without ktlint the fmt metric does not run)")
  command -v detekt >/dev/null 2>&1 || missing+=("detekt -- install: download detekt-cli from https://github.com/detekt/detekt/releases and add it to PATH (Linux) / 'brew install detekt' (macOS) (without detekt the lint metric does not run)")
...
if [ "$QG_ABSOLUTE_MODE" = "1" ]; then
  ...
  if ! qg_resolve_deps "." "$QG_LOG_DIR_ARG/abs-deps.log"; then
    echo "::error::failed to resolve kotlin dependencies -- see $QG_LOG_DIR_ARG/abs-deps.log" >&2
    exit 2
  fi
...
=== Quality Gate (fast-path) ===
  branch:        $branch
  base ref:      $QG_BASE_REF_ARG
  scope:         no Kotlin files touched -- skipping heavy gates
...
if [ -z "$QG_BASELINE_DIR_ARG" ]; then
  ...
  cache_root="${QG_BASELINE_CACHE_DIR:-/tmp/qg-baseline-kotlin}"
...
# Language absent in baseline (sentinelas: build.gradle*)
...
EOF
    ...
    echo "::warning::language absent in baseline (no build.gradle/build.gradle.kts in $QG_BASELINE_DIR_ARG) -- gate skipped" >&2
  fi
  exit 0
fi
...
if ! qg_resolve_deps "$QG_BASELINE_DIR_ARG" "$QG_LOG_DIR_ARG/base-deps.log"; then
  echo "::error::failed to resolve kotlin dependencies (baseline) -- see $QG_LOG_DIR_ARG/base-deps.log" >&2
  exit 2
fi
if ! qg_resolve_deps "." "$QG_LOG_DIR_ARG/pr-deps.log"; then
  echo "::error::failed to resolve kotlin dependencies (PR) -- see $QG_LOG_DIR_ARG/pr-deps.log" >&2
  exit 2
fi
