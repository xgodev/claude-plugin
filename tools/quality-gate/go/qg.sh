...
GO_PATH_RE='\.go$|^go\.(mod|sum)$'

if [ -n "$QG_YAML_EXTRA_FAST_PATH" ]; then
  GO_PATH_RE_EXTRA="${GO_PATH_RE}${QG_YAML_EXTRA_FAST_PATH}"
else
  GO_PATH_RE_EXTRA="$GO_PATH_RE"
fi
...
if [ -z "$QG_BASELINE_DIR_ARG" ]; then
  ...
  cache_root="${QG_BASELINE_CACHE_DIR:-/tmp/qg-baseline-go}"
