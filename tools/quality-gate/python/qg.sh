...
#
# python/qg.sh -- Quality Gate for Python
#
...
show_help() {
  cat <<'EOF'
Usage: python/qg.sh [--base <git-ref>] [options]
     python/qg.sh --detect
...
check_prereqs() {
  local missing=()
  command -v python3 >/dev/null 2>&1 || missing+=("python3 (Python 3.10+) -- install: 'apt install python3 python3-pip' (Linux) / 'brew install python' (macOS) (without Python there is no way to measure the project)")
  command -v ruff >/dev/null 2>&1 || missing+=("ruff -- install: 'pip install ruff' (Linux) / 'brew install ruff' (macOS) (without ruff the fmt/lint metrics do not run)")
  command -v pytest >/dev/null 2>&1 || missing+=("pytest -- install: 'pip install pytest pytest-cov' (Linux) / 'pip install pytest pytest-cov' (macOS) (without pytest the test/coverage metrics do not run)")
  command -v radon >/dev/null 2>&1 || missing+=("radon -- install: 'pip install radon' (Linux) / 'pip install radon' (macOS) (without radon the complexity metric does not run)")
  ...
  # pytest-cov is an importable module; it has no binary. Detect via python.
  if command -v python3 >/dev/null 2>&1; then
    python3 -c "import pytest_cov" 2>/dev/null || missing+=("pytest-cov -- install: 'pip install pytest-cov' (Linux) / 'pip install pytest-cov' (macOS) (without pytest-cov the coverage metric does not run)")
  fi
...
if [ "$QG_ABSOLUTE_MODE" = "1" ]; then
  ...
  if ! qg_resolve_deps "." "$QG_LOG_DIR_ARG/abs-deps.log"; then
    echo "::error::failed to resolve python dependencies -- see $QG_LOG_DIR_ARG/abs-deps.log" >&2
    exit 2
  fi
...
PY_PATH_RE='\.py$|^pyproject\.toml$|^requirements.*\.txt$|^setup\.(py|cfg)$'

if [ -n "$QG_YAML_EXTRA_FAST_PATH" ]; then
  PY_PATH_RE_EXTRA="${PY_PATH_RE}${QG_YAML_EXTRA_FAST_PATH}"
else
  PY_PATH_RE_EXTRA="$PY_PATH_RE"
fi
...
if [ "$QG_FORCE_FULL_ARG" != "1" ]; then
  ...
  if [ -n "$changed_files" ] && ! echo "$changed_files" | grep -qE "$PY_PATH_RE_EXTRA"; then
    branch=$(git rev-parse --abbrev-ref HEAD 2>/dev/null || echo "<detached>")
    ts=$(date -u +"%Y-%m-%dT%H:%M:%SZ")
...
=== Quality Gate (fast-path) ===
  branch:        $branch
  base ref:      $QG_BASE_REF_ARG
  scope:         no Python files touched -- skipping heavy gates
  override:      QG_FORCE_FULL=1 para rodar gate completo

-- modified files --
$(echo "$changed_files" | sed 's/^/  /')

OK fast-path passed (nenhum Python para medir)
EOF
    fi
...
if [ -z "$QG_BASELINE_DIR_ARG" ]; then
  ...
  cache_root="${QG_BASELINE_CACHE_DIR:-/tmp/qg-baseline-python}"
...
# Language absent in baseline (sentinelas: pyproject.toml, setup.py, requirements.txt)
if [ ! -f "$QG_BASELINE_DIR_ARG/pyproject.toml" ] \
   && [ ! -f "$QG_BASELINE_DIR_ARG/setup.py" ] \
   && [ ! -f "$QG_BASELINE_DIR_ARG/setup.cfg" ] \
   && ! ls "$QG_BASELINE_DIR_ARG"/requirements*.txt >/dev/null 2>&1; then
  branch=$(git rev-parse --abbrev-ref HEAD 2>/dev/null || echo "<detached>")
  ts=$(date -u +"%Y-%m-%dT%H:%M:%SZ")
  if [ "$QG_FORMAT_ARG" = "json" ]; then
    cat <<EOF
...
EOF
    ...
    echo "::warning::language absent in baseline (no pyproject.toml/setup.py/setup.cfg/requirements*.txt in $QG_BASELINE_DIR_ARG) -- gate skipped" >&2
  fi
  exit 0
fi
...
if ! qg_resolve_deps "$QG_BASELINE_DIR_ARG" "$QG_LOG_DIR_ARG/base-deps.log"; then
  echo "::error::failed to resolve python dependencies (baseline) -- see $QG_LOG_DIR_ARG/base-deps.log" >&2
  exit 2
fi
if ! qg_resolve_deps "." "$QG_LOG_DIR_ARG/pr-deps.log"; then
  echo "::error::failed to resolve python dependencies (PR) -- see $QG_LOG_DIR_ARG/pr-deps.log" >&2
  exit 2
fi
