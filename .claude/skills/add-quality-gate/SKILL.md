...
**Path convention:** the gate bundle lives under `tools/quality-gate/` in this repo. Every gate-relative path in this skill -- the dispatcher `qg`, `<lang>/qg.sh`, `<lang>/lib/`, `<lang>/rules/`, `<lang>/test-fixtures/`, `<lang>/README.md`, `tests/*.bats` -- resolves inside `tools/quality-gate/`. Repo-root paths (`docs/`, `skills/`, `hooks/`) are written explicitly.
...
bundled dispatcher `qg`, which runs `<lang>/qg.sh --detect` and runs the
