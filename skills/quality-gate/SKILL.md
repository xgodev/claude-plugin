...
usuario. O gate vive em **outro repositorio** e e clonado no diretorio
home do desenvolvedor.

A deteccao de linguagem NAO e mais responsabilidade desta skill nem da
IA. O dispatcher `~/.quality-gate/qg` detecta a(s)
...
- `git` instalado.
- Acesso a `github.com/xgodev/quality-gate`
  (HTTPS ou SSH).
- Pre-requisitos de cada linguagem suportada (instalados pelo proprio
  gate ou documentados em `~/.quality-gate/<lang>/README.md`).

## Fluxo (passos obrigatorios — nao pular)

### 1. Garantir cache local do gate

Sempre verificar se o gate esta clonado em `~/.quality-gate/`:

```bash
if [ ! -d "$HOME/.quality-gate" ]; then
  git clone git@github.com:xgodev/quality-gate.git \
    "$HOME/.quality-gate"
fi
```

Atualizar **em TODA execucao** (nao a cada 24h) -- o repo e pequeno, o
fetch e instantaneo, e isso garante que o gate usado e SEMPRE o `main`
mais recente (sem janela de staleness):

```bash
git -C "$HOME/.quality-gate" fetch --quiet origin
git -C "$HOME/.quality-gate" pull --ff-only --quiet
```

Se `git pull --ff-only` falhar (cache divergiu — alguem editou o gate
local), **PARE** e reporte:

> "Cache local em `~/.quality-gate` divergiu do remote. Resolva
> manualmente (`git -C ~/.quality-gate status`) ou setei
> `QG_PATH=/path/local/dev` para apontar para outra copia."

**Override de desenvolvedor:** se a env var `QG_PATH` esta
setada, use esse path direto e **nao** clone/atualize. Util para quem
esta editando o proprio gate.
...
GATE_PATH="${QG_PATH:-$HOME/.quality-gate}"
...
| "A linguagem X ja deve ser suportada." | Atualize o cache e re-rode o dispatcher. Se exit 3 persiste, PARE e oriente issue. |
...
- Cache em `~/.quality-gate/` atualiza em TODA execucao
  (`git fetch + pull --ff-only`) -- sem janela de staleness. Override
  para dev local: env `QG_PATH`.
...
- `~/.quality-gate/docs/contract.md` — contrato CLI, dispatcher,
  tamper-resistance, `.qg.yaml projects:`.
- `~/.quality-gate/docs/output-format.md` — formato JSON/texto,
  incluindo o envelope de monorepo (`aggregate_verdict`/`results`).
- `~/.quality-gate/docs/consume.md` — como usar localmente.
- `~/.quality-gate/<lang>/README.md` — pre-requisitos da linguagem.
